...
impl HevcPayloader {
    ...
    fn emit(&mut self, nalu: &Bytes, mtu: usize, payloads: &mut Vec<Bytes>) {
        ...
        // Roomler patch (2026-04-20): upstream drops every NAL whose
        // type isn't in the hardcoded UnitType list (i.e., 0, 1, 19,
        // 21, 32, 33, 34, 39). That silently discards IDR_N_LP (20),
        // TRAIL_R (2) and most slice types that MF / HW HEVC encoders
        // legitimately emit, producing a bitstream with parameter
        // sets and occasional slices but no complete IVFs. Only truly
        // reserved types (≥48 except 48/49 for AP/FU payload
        // headers) should be dropped — everything else goes through
        // the generic single-NAL / FU-A path with the original
        // nal_unit_type preserved in the FU header.
        if payload_nalu_type >= 48 && payload_nalu_type != 48 && payload_nalu_type != 49 {
            return;
        }
            ...
            (&self.vps_nalu, &self.sps_nalu, &self.pps_nalu)
        {
            ...
            if aggr_nalu.len() <= mtu {
                ...
                self.sps_nalu.take();
                self.pps_nalu.take();
                // Roomler patch (2026-04-20): upstream unconditionally
                // returns here, silently dropping the CURRENT NAL (the
                // IDR / P-frame that triggered aggregation). Chrome
                // then receives an AP packet with VPS+SPS+PPS but no
                // picture data, so framesAssembledFromMultiplePackets
                // stays 0 and the decoder PLI-storms. Only skip the
                // fall-through emit when the current NAL is itself a
                // param set already included in the AP; otherwise the
                // current NAL must be emitted as its own packet.
                if matches!(
                    nalu_type,
                    UnitType::VPS | UnitType::SPS | UnitType::PPS
                ) {
                    return;
                }
                // Fall through to single-NAL / FU-A path below.
            }
        ...
        while nalu_data_remaining > 0 {
            ...
            +---------------+
            */
            // Roomler patch (2026-04-20): the upstream code only
            // emits an FU header byte when nalu_type is IDR/PFR/BFR
            // (from the hardcoded UnitType list). For any other NAL
            // type (IDR_N_LP=20, CRA=21, TRAIL_R=2, TSA/STSA/RADL
            // variants, ...) *no FU header* is written, producing a
            // malformed FU-A packet that Chrome silently drops during
            // depacketization. Replace the type-table with a generic
            // bit assembly that preserves the original nal_unit_type
            // in the FU header's fu_type field (6 low bits).
            let fu_type = payload_nalu_type & 0x3F;
