...
### P4b — as built (#PR4B): what the first roll found on jupiter
...
| P4 | VAAPI on Linux x86_64 | `ROOMLERD_USE_FFMPEG=0` / the denylist / `vaapi_device` | **shipped** #1494 (+ #1495) → `agent-v0.4.86`: WSL and mars measured as the NEGATIVE cells; jupiter's `encoder-smoke` proved the pump (real HEVC bytes through `radeonsi`) but the PROBE died on drop ⇒ **P4b** #PR4B (`agent-v0.4.87`): the unfed-encoder flush removed, the denylist gating 4:2:0 too; the positive cell on jupiter/zeus follows that roll; Intel (iHD) has no fleet host yet |
